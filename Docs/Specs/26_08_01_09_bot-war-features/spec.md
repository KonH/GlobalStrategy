...
**Cadence:** every **`botDecisionIntervalHours` (default 4)** in-game hours the bot may **draw and/or discard (0+ cards), then play** (play may land on the same tick or a later frame if logic flow requires). This replaces once-per-calendar-day strategic ticks and the prior “acquisition every update / Tick on interval” split.
...
| Scoring arbitration | On the same decision cycle, pick the **global best estimated Δorg_score** among `control` + war + unlock candidates; unlocks sit in the **same scoring pool** (no separate pre-pass) |
...
| Revenge | Only when still **profitable**; packaged **inside `warDeclare`** with a **higher priority bonus** vs ordinary `declare_war` |
...
| Decision cadence | Every **`botDecisionIntervalHours` (default 4)** the bot may **draw and/or discard (0+ cards), then play** (play may be same tick or next frame if required). Lives inside **child B**. Setting: root `game_settings.json` key `botDecisionIntervalHours: 4` |
...
| Eval horizon | Explicit **`endDate` required** (null → ConsoleRunner `StartYear + 5`, not a long scenario). Owner: **`endDate` 1920**. `seedCount` / `hoursPerTick` — see Ambiguities |
...
- Strategic play is still gated once per calendar day via `_lastActedDate` in `Bot.cs`, and acquisition can run every update while pending.
  - Cadence becomes: every **`botDecisionIntervalHours` (default 4)** the bot may **draw and/or discard (0+ cards), then play** (same tick or later frame if required) => bots are not limited to one strategic play per calendar day, and draw/discard are part of the same interval cycle (not “acquisition every update”).
...
- Bot has playable `declare_war` and/or `declare_revenge_war` with a named rival, can afford it under gold policy, expected net Δorg_score is positive after dual-side accounting (including occupation + enemy control decrease + gold), and `WarWinChanceEstimator` win % is acceptable under the chosen threshold; soft target preference favors enemy-higher-control / no-bot-control countries.
  ...
  - When both `declare_revenge_war` and ordinary `declare_war` are profitable candidates, revenge gets a **priority bonus** over ordinary declare => rematches are preferred when still score-positive.
...
- Each registered war-related feature id gets `Docs/BotFeatures/<featureId>/` eval config mirroring `Docs/BotFeatures/control/` (`seedCount: 10`, `hoursPerTick: 24`, `endDate: null` today; opponentFeatures currently `baselineCardPlay` in that file).
  ...
  - Horizon: explicit **`endDate` 1920** (required because null → StartYear+5 only); `seedCount` / `hoursPerTick` per Ambiguities; decision cadence follows `botDecisionIntervalHours` (default 4).
...
- **Discard today:** `IBotCommandSink.DiscardCountryCard` exists; **only** `ControlFeature.TryDiscardForBetterHand` calls it when hand full and no control play. Cost: `GameSettings.DiscardGoldCost` (50). Owner requires a **common shared** discard helper on the 4h cycle.
...
| `declare_revenge_war` | Revenge | 125 | control ≥20; mil ≥25; warFree; revengeEligible | `warDeclare` (priority bonus vs ordinary declare) |
...
| **B** Decision interval (`botDecisionIntervalHours` **inside B**) + shared discard + intent-aware draw scoring + cross-feature Δorg_score arbitration; 4h cycle = draw/discard then play | `/specify`+`/plan` |
| **C** `warUnlock` (`make_rival`, opinion cards) | `/implement-bot-feature` after A–B |
| **D** `warDeclare` (`declare_war` + `declare_revenge_war` with **priority bonus**; dual-side profit; soft target bias) | `/implement-bot-feature` after A–B |
...
| **F** Eval configs under `Docs/BotFeatures/<id>/` (`endDate` 1920; success = war plays + beat control-only twin) | `/implement-bot-feature` configs carve-out |

Parent #83 stays the umbrella until children are filed; do not implement the whole stack in one PR. Next step after specify approval: `/plan` on the **first approved child** (A), not the whole umbrella in one go.

### Stale vs prior revision of this spec

- Locked eval primary gate = control-only twin; secondary not required for merge.
- Locked gold dip policy (ii) for high-EV war spends.
- Decision cycle revised: **4h draw/discard-then-play** (not acquisition-every-update + Tick-on-interval).
- Eval `endDate` explained (null → StartYear+5) and locked to **1920**; `hoursPerTick` / `seedCount` remain Ambiguities.
- Occupation depth locked to **full** per-country counts.
- Child split A–F approved; interval inside B; revenge inside D **with priority bonus**; soft target bias locked.
- Force cards still out of v1; sell_arms → natural peace unchanged.
- Success bar still (a)+(b).

## Ambiguities

- [NEEDS CLARIFICATION: Eval sim step — with `botDecisionIntervalHours: 4` and `endDate` 1920-01-01 (40 in-game years from 1880), should war evals use **`hoursPerTick: 24`** (cheaper; bot still gets a decision opportunity each eval step because 24h ≥ 4h) or **`hoursPerTick: 4`** (matches decision interval exactly, but ~6× more ticks for the same calendar span)? Also confirm **`seedCount: 20`** (vs control’s 10). Assumed: `hoursPerTick: 24`, `seedCount: 20`, `endDate: 1920-01-01`.]
- [NEEDS CLARIFICATION: 4h decision-cycle details — after the draw/discard phase, is strategic play still **at most one** card play per interval (global-best arbitration among feature proposals)? Is discard count per interval unbounded (0+, until hand has a suitable play or gold/hand constraints stop it), or is there a hard cap? When play may be “next frame,” is it acceptable to **receive a drawn card and play on the same `ExecuteDecisionTick`** if world state already allows, deferring to a later tick/frame only when command/apply ordering requires it? Assumed: one strategic play per interval; unbounded discards until useful or blocked; same-tick play OK when state allows.]
- [NEEDS CLARIFICATION: Revenge priority bonus — implement as a **soft additive weight** on estimated Δorg_score for `declare_revenge_war` vs ordinary `declare_war` (concrete magnitude chosen at `/plan`), or a **hard rule** “prefer revenge whenever both are score-positive”? Assumed: soft additive weight; `/plan` picks the number.]
