...
Source: `Docs/Specs/26_07_11_10_province-ownership/spec.md`.
...
- [ ] **Remove `MapRenderer`'s feature path as dead code** — Delete `Assets/Scripts/Unity/Map/MapRenderer.cs` and its only collaborator `Assets/Scripts/Unity/Map/FeatureIdentifier.cs` (both `.cs` + `.meta` via Bash `rm`, per project convention). Update `Map.cs` (drop `_renderer`/`Renderer`, `Render` call, and the `MapRenderer` param of `Initialize`), `MapController.cs` (drop `ActiveRenderer`), and `MapLoader.cs` (drop the `MapRenderer` wiring). `MapRenderer`/`ActiveRenderer`/`FeatureObjects` have two more real consumers beyond the Map scene that must be migrated in the same step or compilation breaks:
...
Switch between Province and Political/Org/Geographic lenses. Confirm: the Province lens still shows the gray province borders (spec 26_07_11_09_province-map-lens) and country lenses show no province borders and look identical to today; country fills match the owning country's colour.
