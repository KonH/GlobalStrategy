...
2. [x] **Sanity-check scores still populate in Play mode** — via `VisualState.CountryScore.ScoreByCountryId` (no consumer UI exists yet, per spec 26_07_14_09_country-scoring's scope — confirm through a debugger/log or a temporary breakpoint) that country scores are still non-zero and correct after this refactor, matching pre-refactor behavior. Verified via `execute_code` in Play mode on `Map.unity`: 20 countries with non-zero scores (e.g. `Argentina=2250.15`, `Manchu_Empire=113466.95`).
...
- *File organisation.* Plan lives at `Docs/Specs/26_07_16_15_score-component-composition/plan.md`, whose commit-derived timestamp precedes `Docs/Specs/26_07_16_09_org-scoring/` because org-scoring depends on this refactor landing first. It uses the dated `Docs/Specs/<YY_MM_DD_HH>_<name>/` subdirectory layout even though there is no accompanying `spec.md`.
