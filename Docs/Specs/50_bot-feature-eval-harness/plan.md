...
Plan 48's per-org DTO (`SimulationResult`'s org entry, used by both end metrics and timeline samples) gains `public double Score { get; set; }` → serialized `score` (camelCase policy already in place). `HeadlessRunner` computes it via `OrgScore.GetScore(logic.World, orgId)` at exactly the existing sampling moments: the t0 baseline sample (valid: spec 47 forces `CountryScoreSystem.Recompute` in `InitSystem.Run`) and each month-boundary sample taken after `logic.Update` (valid: `CountryScoreSystem.Update` recomputes inside that same `Update`, after population growth), plus the end-metrics block. Additive only — part 1/2-shaped consumers ignore the extra field; determinism is preserved (score derives only from world state). If `HeadlessRunner` does not already expose the world read-only, add an `IReadOnlyWorld` accessor on `GameLogic` usage it already has (plan 48's runner reads world state for `OrgMetrics`; reuse the same access path).
...
**Attribution.** Spec 49 fixes that the runner's bot host ticks features one at a time; the host therefore knows the currently-deciding feature without the sink knowing it. In the ConsoleRunner bot loop, before each `feature.Tick(...)` the host sets a local `currentFeatureId`; the callback closure stamps each emission with `{ orgId (the bot being ticked), currentFeatureId, actionId, countryId, gameDate, tick }` where `gameDate` is the current game date (`yyyy-MM-dd` from `GameTime.CurrentTime`) and `tick` is the runner's tick counter. No wall-clock anywhere. The callback fires synchronously inside `Tick`, so the stamp can never mis-attribute across features.
...
**In-process invocation.** `Game.Evals` builds one `HeadlessOptions`-equivalent request per run (seed, orgs, config dir, end date, hours per tick, timeout, bot profiles as constructed `BotProfile` DTOs — spec 49 guarantees a DTO-accepting in-process path since JSON parsing is ConsoleRunner-side only) and calls the same `HeadlessRunner` entry `Program.Main` uses, receiving the `SimulationResult` in memory and writing the per-run JSON itself into the attempt dir. If plan 49 lands the runner entry as file-path-profiles only, the fallback is equally deterministic: `Game.Evals` writes the effective profile JSONs into `<attempt>/profiles/` (it does this anyway, for auditability) and passes the paths. No per-run process spawn either way.
...
| `2` | validation error before any run — unknown `featureId`, malformed/missing explicit config, run cap exceeded, candidate org not in `organizations.json` |
...
5. **Write `.ralph/prd.md` directly** (no `/specify`, no `/plan`, no `/create-prd`), using the existing task shape `{category, description, steps, gate, passes}`, and reset `.ralph/activity.md` to its header. Tasks, in order:
   ...
   3. Run the eval batch — gate: `dotnet run --project src/Game.Evals -- --feature <featureId>` (the CLI's exit code **is** the verdict; never a fabricated always-green gate). Task `steps` encode the improvement loop: on failure, read `Docs/BotFeatures/<featureId>/eval_history.json` and `.ralph/activity.md`, pick **one** concrete improvement (logic change, different targeting, adjusted parameter ranges in the eval config), journal it, and re-run; if `eval_history.json` already holds **5** attempts for this branch and the gate still fails, journal budget exhaustion, leave `"passes": false`, and end the iteration — the 5-attempt budget lives here in the PRD (and is restated in the skill), independent of the driver's `MaxIterations`.
...
- `src/Game.Bots`' bot-facing surface (`IBotFeature`, `IBotObservation`, `IBotCommandSink` interface, profile schema, whitelist) — the only addition is the optional emission callback on the sink *implementation* (§4).
