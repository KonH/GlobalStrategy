...
- The app is opened in a browser (screen A — main menu).
  ...
  - CONTINUE is activated => the most recent save loads directly into the game view (screen C) with its organization, game date, and game state restored — no save-picker appears; language and auto-save interval still follow the app's own settings, not values stored in the save.
...
- The settings screen is open.
  ...
  - Settings are changed and a game is later loaded via CONTINUE => the session uses the app settings; nothing stored inside the save overrides them (settings are their own persistent state, separate from saves).
...
- Any screen of the app is shown (A, B, C, or D).
  - The page header renders => a warning is visible stating this is an experimental version, with a link to the full client (the Unity WebGL demo the README links to); activating the link opens the full client.
...
- Tech stack decision — **Blazor WebAssembly standalone (.NET 8), whole simulation in-browser, static-file deploy**:
  ...
  - Deployment: `dotnet publish -c Release` emits a fully static site; a new GitHub Actions workflow (the repo has no `.github/` yet) publishes it on push to the default branch via `actions/upload-pages-artifact` + `actions/deploy-pages`. Target is **this repository's own project Pages site** (`https://konh.github.io/GlobalStrategy/`, `<base href>` rewritten to `/GlobalStrategy/`) — a second Pages site, fully separate from the owner's existing GitHub Pages site, which stays untouched. "Update" = merge + automatic republish; "new commands appear" = the same publish carries the freshly built game assemblies.
...
- Main menu, CONTINUE, and saves in the browser (screens A/D):
  ...
  - The snapshot serializer is a web-side `ISnapshotSerializer` using Newtonsoft.Json like `Assets/Scripts/Unity/Save/NewtonsoftSnapshotSerializer.cs`, serializing the same `WorldSnapshot` model (`src/Game.Main/WorldSnapshot.cs`); Newtonsoft already runs outside Unity (`Game.ConsoleRunner` takes the package). Cross-client save compatibility is explicitly NOT a requirement — web saves live and die in the browser; the shared model is reuse, not a format contract.
...
- Settings and localization:
  - Settings are their own persistent state, fully separate from saves: language and auto-save interval live as web-local preferences (localStorage). On EVERY session start — new game and CONTINUE alike — the client pushes `ChangeLocaleCommand(locale)` and `ChangeAutoSaveIntervalCommand(interval)` on the first tick (the same commands `Assets/Scripts/Unity/UI/SettingsWindowDocument.cs` pushes; interval strings `"daily"`/`"monthly"`/`"yearly"`), deliberately overriding whatever `AppSettings` values a loaded save carries. Menu-screen language switching is applied client-side immediately, mirroring the `StaticGameLogic` pattern for pre-game scenes.
...
- Page header and footer (all screens):
  - A shared layout component wraps every screen. Header: an experimental-version warning plus a link to the full client — the Unity WebGL demo URL from `README.md` (`https://play.unity.com/en/games/790490b4-2b09-4c1c-8a2e-df23f6b43b47/global-strategy`). Footer: the copyright note matching `LICENSE` (`© 2026 KonH · MIT License`) and a link to `https://github.com/KonH/GlobalStrategy`.
  - Header/footer strings use new `web.*` locale keys appended to `Assets/Localization/en.asset` / `ru.asset` (additive; unused by the Unity client), so they follow the language switch like all other UI text.
...
- Terminal — command discovery, execution, and auto-complete:
  ...
  - Project-convention deliverable: a new skill `.claude/skills/add-terminal-command/SKILL.md` (indexed from `CLAUDE.md`, following the existing `.claude/skills/*` layout) that triggers when adding or changing `ICommand` types and mandates: every command parameter carrying a domain id must be annotated with an existing suggestion attribute (or `OneOf`), and introducing a genuinely new id kind means adding the attribute plus its suggestion provider in the same change. A skill is preferred over a `.claude/rules/` file because rules are lazily indexed reference material while skills trigger on the matching task.
...
- Save import/export or any save-compatibility guarantee between browser and Unity clients — web saves are strictly per-browser.
