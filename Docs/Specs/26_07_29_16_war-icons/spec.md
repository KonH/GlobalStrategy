...
- A qualifying war has its current core-model participants (one attacker and one defender).
  - Its button is rendered => it contains the owner-provided crossed-swords icon and the flags of the primary attacker and primary defender.
...
- The player releases the primary pointer over a qualifying war button.
  - The button emits a navigation/open request containing that war's `WarId` => the war progress window integration can open the matching war rather than inferring it from country ids or row position.
  - The request is handled by the war progress window surface => that surface is opened for the requested war.
...
- **Button visuals:**
  - The crossed-swords artwork is an owner-provided UI asset, not generated by this feature. Import it under the established UI/icon asset hierarchy and reference it from the feature USS or a serialized visual config, depending on its supplied format.
...
- **Progress-window handoff:**
  ...
  - `HUDDocument` forwards the view callback to the war progress window/document integration when that surface is available.
  - The progress window's UXML, view model, contents, controls, and game behavior are not part of this feature.
...
- **Crossed-swords source asset is not present:** the live issue says the icon is provided by the user, but the issue body and comment thread contain no attachment or repository path. The implementation plan needs the asset (or an owner-approved path to an already committed asset) before the final button visual can be completed.
- **War progress window integration does not yet exist:** the click behavior is specified as a `WarId` navigation/open request so this feature does not invent the out-of-scope window. Planning should bind that request to the window's actual document/service if it exists by then; otherwise the window feature must expose that contract before end-to-end click navigation can be completed.
