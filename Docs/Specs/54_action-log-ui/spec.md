...
- **Control** (`CreateActionEffectSystem` → `ControlEffect`, aggregated into `CountryControlState` totals by `ControlSystem`) — fully live today via real card actions (e.g. `letter_commendation_control`, `royal_audience_control`). The "to +N" total is already tracked as a running aggregate per org per country, updated whenever a new `ControlEffect` is created — no new game-logic wiring is required, only observing this existing total.
- **Opinion** (`CreateActionEffectSystem` → `OpinionModifierEffectParams` → `ResourceChange`/`CharacterStateEntry.Opinion`) — also fully live today via real card actions (e.g. `letter_commendation_opinion`, `royal_audience_opinion`), contrary to a first-pass assumption that only the debug "Improve Opinion" cheat touches opinion. The character's current effective opinion is already tracked as a value that can increase; no new game-logic wiring is required, only observing that value increase.
...
- **Given** an organization's aggregate control total in a country increases **When** the action log next refreshes **Then** a new line reading `<OrganizationDisplayName> increased control in <CountryDisplayName> to +<N>` appears, where `<N>` is the organization's new resulting control total in that country (not the delta), formatted per the number-formatting rule below.
- **Given** a character's effective opinion of an organization increases **When** the action log next refreshes **Then** a new line reading `<OrganizationDisplayName> increased <CharacterRoleDisplayName> <CharacterDisplayName> opinion in <CountryDisplayName> to +<N>` appears, where `<CountryDisplayName>` is the character's home country, `<CharacterRoleDisplayName>` is the character's role, and `<N>` is the character's new resulting opinion value for that organization (not the delta), formatted per the number-formatting rule below.
- **Given** a new character becomes the occupant of a role slot (country-government role or org role) **When** the action log next refreshes **Then** a new line reading `New <CharacterRoleDisplayName> in <CountryDisplayName> / <OrganizationDisplayName> - <CharacterDisplayName>` appears, subject to the data-model gap noted under Ambiguities.
...
- **Given** a log line contains a numeric value **When** it is rendered **Then** the value is formatted with exactly one digit after the decimal point (e.g. `+3.5`), never as a bare integer (`+3`) and never with more than one decimal digit (`+3.50`), matching the project's existing `:F1`-style convention for fractional game values.
...
- **Given** a log line contains any other text (character display names, connector words like "discovered"/"increased"/"in"/"to") **When** it is rendered **Then** that text uses the panel's default style: white text with a black shadow/outline for legibility over the map, matching every other HUD line's default treatment. Character display names are not bolded or colored unless covered by another rule above.
...
- **Given** a bottom-bar panel (`.country-info-panel`, `.org-lens-country-info-panel`, or the org-lens equivalent) is currently visible **When** the action log panel is positioned **Then** the panel's bottom edge sits immediately above that bottom-bar panel's actual rendered top edge (tracking its live height, since these panels have dynamic, content-driven height and can toggle to `display: none` entirely).
- **Given** no bottom-bar panel is currently visible (nothing selected) **When** the action log panel is positioned **Then** its bottom edge falls back to the screen's bottom edge offset by the same `6px` margin used elsewhere in the HUD (matching `.top-right-panel`'s own offset convention), rather than leaving an undefined or unbounded height.
...
- **Given** `gameLog.maxLogEntries` defines the maximum number of entries the log retains **When** appending a new entry would exceed that cap **Then** the oldest entry is dropped (per the fade-out animation above) so the log never holds more than `maxLogEntries` entries at once. The exact default numeric value is an implementation detail for `/plan` (e.g. mirroring the existing `BotActionLogRetentionCap` precedent in `GameSettings`) unless the user specifies one.
...
- **Log persistence across save/load.** The action log's entry history is not written to the save file and is not restored on load — it is presentation-only state, following the same "not `[Savable]`, rebuilt/derived rather than persisted" convention already used for the discovery-diff state (`DiscoveredCountriesState`). On load, the log starts empty for that session, and the internal baselines used to detect "this value just increased" (control totals, opinion totals, discovered-country set, role-slot occupants) are re-initialized to the freshly loaded state on the first tick after load — this deliberately avoids replaying every historical event in the save as a flood of log lines the moment a save is loaded.
...
- [NEEDS CLARIFICATION: The fourth line format (`New <Role> in <Country> / <Org> - <Character>`) asks for both a country and an organization on the same line, but the current data model (`Character`/`CharacterSlot` in `src/Game.Components/`) is mutually exclusive — a `Character` has either a non-empty `CountryId` with `OrgId == ""` (country-government roles like Ruler, General) or a non-empty `OrgId` with `CountryId == ""` (org roles like Master, Agent); no character today has both populated. Product needs to clarify what should appear in the empty slot for each case: for a country-role appointment, should `<Org>` be omitted, replaced with a placeholder, or resolved to some other org (e.g. the country's controlling organization, if any)? For an org-role appointment, should `<Country>` be omitted or resolved to some org-associated country (e.g. where the org's headquarters/home country is, if such a concept exists)? This spec's acceptance criteria describe the format as requested, but the concrete field-population rule for each variant needs a decision before `/plan` can specify the exact wiring.]
- [NEEDS CLARIFICATION: `gameLog.maxLogEntries` establishes the cap as a config field, but no specific default numeric value was given — if the product wants a specific default (rather than a `/plan`-chosen one, e.g. mirroring `BotActionLogRetentionCap`), it should be stated.]
