  ...
  "actions": [
    {
      ...
      "deckCopies": 3,
      "cooldownDays": 30,
      "conditions": [{ "type": "ResourceCondition", "resourceId": "control", "minValue": 0 }],
      "cost": [],
...
- **Given** `preDealtToHand` was `true` on `sphere_of_pressure` (which has `controlThreshold: 0`), **When** the new logic runs with a `resourceCondition` of `{ "resourceId": "control", "minValue": 0 }`, **Then** `sphere_of_pressure` is dealt to the opening hand because the condition is satisfied at game start (control ≥ 0).
- **Given** a card with `resourceCondition: { "resourceId": "control", "minValue": 10 }`, **When** starting control is 0, **Then** the card is NOT dealt to the opening hand.
...
### 4. `controlThreshold` replaced by DSL-expression `conditions` array
...
- **Given** an action definition, **When** it has play conditions, **Then** they are expressed as a `conditions` array of boolean DSL expressions using the same node format (`type`/`members`) as `successRate`, e.g.:
  ...
  ```
- **Given** a condition expression, **When** evaluated, **Then** it uses the same evaluation context (scope) as `successRate` — `control()` resolves to the player org's control in the target country; `opinion()` resolves to the relevant opinion value.
...
- **Given** the old `successRateBase: 0.3, successRateControlDivisor: 2`, **When** converted to the new schema, **Then** the action has:
  ```json
  "successRate": {
    "type": "add",
    "members": [
  ...
  ```
- **Given** `successRateControlDivisor: 0` (divisor unused), **When** converted, **Then** the expression is simply `{ "type": "value", "members": [0.5] }` — no `add` or `div` wrapper.
- **Given** an expression tree at runtime, **When** evaluated, **Then** `value` returns the literal number in `members[0]`; `add` returns the sum of all `members`; `sub` returns `members[0] - members[1]`; `mul` returns the product of all `members`; `div` returns `members[0] / members[1]` (denominator ≠ 0); `clamp` returns `members[0]` clamped between `members[1]` and `members[2]`; `control` returns the player's current control in the target country; `opinion` returns the current opinion value for the relevant relationship.
...
- **Given** the supported op set, **When** authoring expressions, **Then** the following ops are available in this iteration: `value`, `add`, `sub`, `mul`, `div`, `clamp`, `control`, `opinion`.

### 7. Control / opinion outcomes move to effect definitions

- **Given** an action that previously carried `controlOnSuccess: 10`, `opinionModifierSourceId: "letter_of_commendation"`, `opinionModifierValue: 50`, `opinionModifierChangeValue: -1`, **When** the refactor ships, **Then** those fields are absent from the action definition.
...
- **Given** an effect definition of type `ControlChange`, **When** applied, **Then** it carries typed parameters specifying the control delta (exact C# class design is a plan-phase concern).
...
- **Given** `EffectConfig.cs` today, **When** extended, **Then** `ActionEffectDefinition` uses typed subclasses per `effectType` (not a freeform dictionary). Every effect type that carries parameters gets its own typed C# class. Effect types needed in this iteration: at minimum `DiscoverCountry`, `ControlChange`, `OpinionModifier`.
...
- **Given** the `DiscoverCountry` effect type, **When** its parameters are deserialized, **Then** `minCountryChance` (and any control/opinion weighting for country selection) is a typed field on the `DiscoverCountryEffectParams` C# class (or equivalent), not on `ActionDefinition`.
