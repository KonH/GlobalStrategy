...
- [ ] **Step 9 — Update `CountryInfoView.cs`** — In `Assets/Scripts/Unity/UI/CountryInfoView.cs`:
...
- [ ] **Step 10 — Update `HUDDocument.cs` to pass configs to `CountryInfoView`** — In `Assets/Scripts/Unity/UI/HUDDocument.cs`:
  ...
  - In `Start()`, update the `CountryInfoView` constructor call to pass `_countryVisualConfig` (already injected via `CountryVisualConfig` — verify it is already a field; if not, add it) and `_orgVisualConfig` as the two new trailing arguments.
  - Note: `CountryVisualConfig` is already registered as a singleton in `GameLifetimeScope`; `OrgVisualConfig` is also already registered. Neither requires a new registration — only the injection point on `HUDDocument` needs updating.

- [ ] **Step 11 — Update `OrgLensCountryView.cs`** — In `Assets/Scripts/Unity/UI/OrgLensCountryView.cs`:
...
- [ ] **Step 12 — Update `HUDDocument.cs` to pass `OrgVisualConfig` to `OrgLensCountryView`** — In `Assets/Scripts/Unity/UI/HUDDocument.cs`, update the `new OrgLensCountryView(...)` constructor call in `Awake()` to pass `_orgVisualConfig` (already added to the `[Inject]` method in Step 10).

- [ ] **Step 13 — Update `OrgInfoDocument.cs`** — In `Assets/Scripts/Unity/UI/OrgInfoDocument.cs`:
  - Add `OrgVisualConfig _orgVisualConfig;` field.
  ...
  - In `Refresh()`, after setting `_orgName.text = org.DisplayName`, look up `_orgVisualConfig?.Find(org.OrgId)`, get `flag` Sprite, set `_orgFlagElement.style.backgroundImage` and show/hide based on null.
  - `OrgVisualConfig` is already registered as a singleton in `GameLifetimeScope` — no new registration needed.

- [ ] **Step 14 — Update `PlayerOrgView.cs`** — In `Assets/Scripts/Unity/UI/PlayerOrgView.cs`:
...
- [ ] **Step 15 — Update `HUDDocument.cs` to pass `OrgVisualConfig` to `PlayerOrgView`** — In `Assets/Scripts/Unity/UI/HUDDocument.cs`, update the `new PlayerOrgView(...)` constructor call in `Start()` to pass `_orgVisualConfig` as the new trailing argument (added in Step 10).

- [ ] **Step 16 — Update `SelectOrgDocument.cs`** — In `Assets/Scripts/Unity/UI/SelectOrgDocument.cs`:
  - Add `OrgVisualConfig _orgVisualConfig;` field.
  - In `[Inject] void Construct(...)`, add `OrgVisualConfig orgVisualConfig` parameter and assign `_orgVisualConfig = orgVisualConfig`.
  - In `Start()`, after querying `_orgNameLabel`, query `_orgFlagElement = root.Q("org-flag")`; store as `Label _orgFlagElement` (actually `VisualElement _orgFlagElement`).
  ...
  - `OrgVisualConfig` is already registered in `SelectCountryLifetimeScope` — no new registration needed.

- [ ] **Step 17 — Verify `PlayerOrganizationState.OrgId` accessibility** — Search `GS.Main.PlayerOrganizationState` (in the `src/` C# solution or the compiled DLL's public API) to confirm `OrgId` is a public property. If the property name differs, adjust Step 14 accordingly. This is a read-only check — no code change needed if it already exists.

- [ ] **Step 18 — Verify `HUDDocument` already has `CountryVisualConfig` injection** — Read `HUDDocument.cs` fully to confirm whether `CountryVisualConfig _countryVisualConfig` is already a field and injected. The `[Inject]` method currently accepts `CharacterVisualConfig` but not `CountryVisualConfig`. If absent, add `CountryVisualConfig countryVisualConfig` to the `[Inject]` parameter list and store it. `CountryVisualConfig` is already registered in `GameLifetimeScope`.
...
- **VContainer sole DI** — all new dependencies injected via existing `[Inject]` methods or constructor parameters; no `new` calls for shared config objects; `CountryVisualConfig` and `OrgVisualConfig` are already registered singletons in both `GameLifetimeScope` and `SelectCountryLifetimeScope`.
