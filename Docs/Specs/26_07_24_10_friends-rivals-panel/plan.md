...
No `Game.Tests` unit-test surface applies: this feature adds no `[Savable]` components, no `World` mutation, and no new command beyond pushing the already-tested `SelectCountryCommand`. All logic is Unity-side UI Toolkit rendering/event wiring (`CountryInfoView`, `HUDDocument`), which this project does not unit test. Verification is manual, via the User Steps above: block position and header text, hide-when-empty behavior (Friends only, Rivals only, neither, both), tooltip content on hover, click-to-reselect (including sub-panel closing on reselect), stale-flag-free refresh on any selection-change path, and WebGL-safe (no emoji/Unicode glyph) compliance.
...
- **Game Logic (ECS):** No `src/` game-state changes; `CountryRelationsState` already exists and is read-only from the UI side. All new code is Unity-side presentation/input glue (`CountryInfoView`, `HUDDocument`).
