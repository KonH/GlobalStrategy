...
Add `using GS.Game.Systems;` to `InitSystem.cs`/`VisualStateConverter.cs` if not already present (both already reference other `GS.Game.Systems` helpers, so likely already imported — verify during implementation).
