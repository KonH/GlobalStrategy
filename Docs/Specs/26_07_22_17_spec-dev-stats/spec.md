...
| `spec_size_tokens`, `plan_size_tokens` | **Amendment:** rough token-count estimate for the same snapshot (`scripts/stats/token_estimate.py`, stdlib-only ~4-chars/token heuristic — no real tokenizer dependency, to keep the SessionEnd hook network/API-key-free); an approximation for cross-spec size comparison, not a billing-accurate count; same empty-cell-means-missing-file convention |
