...
**`usage.csv` columns** (verbatim from spec): `spec_id`, `version` (bundleVersion from `ProjectSettings/ProjectSettings.asset`), `stage`, `mode` (`automated`|`interactive`), `context` (`fresh`|`continued`), `start`, `end` (UTC ISO 8601), `provider` (`claude`|`codex`), `model`, `cost_usd`, `input_tokens`, `cached_input_tokens`, `output_tokens`, `spec_size_kb`, `spec_size_tokens`, `plan_size_kb`, `plan_size_tokens`, `prd_size_kb`, `diff_lines`, `session_id`. **Amendment:** `spec_size_tokens`/`plan_size_tokens` added after their `_kb` counterparts — a stdlib ~4-chars/token estimate (`scripts/stats/token_estimate.py`), not a real tokenizer, so every row write still runs standalone with no network/API key.
...
scripts/stats/
  ...
  token_estimate.py      # Amendment: stdlib ~4-chars/token estimate for spec_size_tokens/plan_size_tokens
  ...
  tests/
    ...
    test_csv_store.py
    test_watermark.py
    test_token_estimate.py  # Amendment: spec_size_tokens/plan_size_tokens heuristic
```
...
- **Amendment (post-implementation):** a `type:"assistant"` line invoking the `Skill` tool with `input.skill` equal to `specify`/`plan`/`implement` (optionally plugin-prefixed, e.g. `k:specify`; anchored so names like `implement-bot-feature` never match) also starts the corresponding stage. Remote (Claude Code on the web) and skill-invoked sessions never get a `<command-name>` marker — the CLI injects it only for typed slash commands — so without this those sessions produced zero rows. A repeated Skill invocation of the already-current stage (e.g. a retry after an error) does not restart the stage. Sub-agent transcripts still aren't visible to the scan (they live outside `~/.claude/projects/<slug>/` on remote runners), so rows from such sessions undercount by the sub-agents' usage; `--record` remains the manual correction path.
...
- [x] **Amendment: add `spec_size_tokens`/`plan_size_tokens`** — `token_estimate.py` (stdlib ~4-chars/token heuristic, no tokenizer dependency), wired into `build_row` right after the corresponding `_kb` columns, with `test_token_estimate.py` and an updated `test_column_order_matches_spec_exactly`. Backfilled onto every existing `usage.csv` row (including the pre-existing spec-dev-stats backfill from `Docs/Specs/26_07_22_17_spec-dev-stats/`'s own out-of-scope note) via a one-off `.tmp/migrate_size_tokens.py`, using each spec's current on-disk `spec.md`/`plan.md` (same "current snapshot" convention the `_kb` columns already use for backfilled rows).
