...
As a game designer, I want every country that is currently a war participant (per war-mechanics-core, GitHub issue #69 / `Docs/Specs/26_07_25_06_war-mechanics-core`) to expose two country-scoped wartime resources — `damage` and `durability` — derived from a per-country config baseline (1–100, reflecting 1880-era war technology) plus the relevant ruler and advisor character skills, and kept correct whenever those characters or skills change, so that a later battle/combat slice can read stable offensive and defensive war strength without re-deriving skill math itself.
...
- **Given** war-mechanics-core is available (`Wars.IsInWar` / `WarParticipant`) and a country is not a participant in any war **When** that country's resources are queried **Then** it has no live country `Resource` entities with `ResourceId` `damage` or `durability` (those resources exist only while the country is at war).
- **Given** two countries enter a war (e.g. via `DebugDeclareWarCommand` or any path that creates `WarParticipant` rows) **When** each country becomes a `WarParticipant` **Then** that country immediately gets country-owned `[Savable]` `Resource` entities for `damage` and `durability`, plus Instant `ResourceEffect` + `ResourceCollector` entities that set each resource to `base + skillA + skillB` (see Tech Notes).
- **Given** a country is at war **When** its `damage` is computed **Then** `target = baseDamage + rulerPower + militaryAdvisorPower`, where `baseDamage` is that country's authored config integer in `[1, 100]`, `rulerPower` is the `power` skill `Resource` on the country's `ruler` character (or `0` if missing), and `militaryAdvisorPower` is the `power` skill on the `military_advisor` character (or `0` if missing). Theoretical max is `300` when base and both skills are `100`; there is no clamp above base when skills are present, and missing skills can leave the final value equal to or above base only by the skills that exist (final can be below a naïve "base + two skills" expectation, and equal to base when both skills are missing).
- **Given** a country is at war **When** its `durability` is computed **Then** `target = baseDurability + rulerStinginess + economicAdvisorStinginess`, with the same additive shape, using config `baseDurability` in `[1, 100]` and the `stinginess` skill on roles `ruler` and `economic_advisor` (missing → `0`).
...
- **Given** a country is at war and its ruler, military advisor, and/or economic advisor is replaced (e.g. `DebugCycleCharacterCommand`) **When** the character change is applied **Then** collectors reading live skill resources re-derive the affected war resource(s) so `damage` / `durability` match the new characters' current skills (not deferred to a month boundary).
- **Given** a country is at war and a contributing skill `Resource.Value` mutates in place on the assigned ruler or relevant advisor **When** that mutation is applied **Then** the same collectors re-derive the affected war resource(s) from the live skill values.
...
- **Given** a country stops being at war (e.g. `DebugStopWarCommand` / `Wars.StopWar`) **When** its `WarParticipant` is removed **Then** that country's `damage` and `durability` `Resource` entities and their linked Instant `ResourceEffect` + `ResourceCollector` entities are destroyed for both former participants.
...
- **Given** damage and durability exist for a country at war **When** any other system or a future battle slice needs them **Then** they are queryable as ordinary country `Resource` values (same style as gold / recruits / `country_score`) without the consumer re-deriving skill math. No battle consumer is implemented in this feature.
- **Given** a save is loaded while a war is active **When** init/load completes **Then** each war participant's `[Savable]` `damage` / `durability` resources are present (persisted like gold) and collectors re-sync absolute values from current config bases + current character skills on the next Instant resolve pass / bootstrap so values are not stale relative to skills.
- **Given** a country at war is selected **When** `VisualState.SelectedCountry.Resources` is built **Then** `damage` and `durability` appear automatically via existing `BuildResources` (no special display whitelist work in this feature; dedicated War window UI is out of scope).
...
- **Lifecycle (locked):** Unlike gold (always present), these are wartime-only. On `WarParticipant` create for a country: create the two `Resource` entities (initial `Value = 0` or equivalent) and Instant `ResourceEffect` + `ResourceCollector` entities that absolute-rederive via `delta = target - currentValue`. On war end / participant destroy: destroy those resources and their linked effect/collector entities. On save/load while at war: resources persist; collectors re-sync from live skills on Instant resolve / bootstrap.
- **Collectors (locked):** Register Instant absolute collectors (same delta contract as `country_score` / `CountryPopulationCollector` / `OrgScoreCollector`: `return target - currentValue`). Collectors read live skill `Resource` values on the current characters for that country, so character cycle and in-place skill mutation are covered whenever the collector runs. Pair with the existing Instant seed + recurring dual-effect pattern (`AttachCollectorDrivenCountryEffects` / org_score Instant+Daily) as needed so mid-war skill changes stay correct after the Instant effect self-destructs — without inventing a second pipeline.
