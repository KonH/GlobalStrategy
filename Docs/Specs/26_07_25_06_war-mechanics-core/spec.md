...
- **`src/Game.Systems/Wars.cs`** (new static class, same shape/style as `src/Game.Systems/CountryRelations.cs` — a plain helper, not a "System": per `.claude/rules/unity/ecs_patterns.md`'s no-system-to-system-calls rule, a "System" suffix is reserved for classes whose only public surface `GameLogic` calls is a single per-tick `Update`; `DeclareWar`/`StopWar` are invoked directly from `GameLogic`'s debug command loops, the same way `CountryRelations.SetRelation`/`RemoveRelation` are, so they live on a non-"System" helper rather than being mixed onto `WarSystem`):
  ...
  - None of these methods call another system's entry point, consistent with the no-system-to-system-calls rule.
- **`src/Game.Systems/WarSystem.cs`** (new static class, same shape/style as `src/Game.Systems/ControlSystem.cs` — a real system whose only public entry point is `Update`, called once per tick from `GameLogic`):
  ...
  - `Update` does not call another system's entry point, and nothing outside the top-level orchestrator calls `WarSystem.Update` — consistent with the no-system-to-system-calls rule.
