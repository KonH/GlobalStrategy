...
- **Given** the transient "recent event" fields — `DiscoveredCountriesState.RecentlyDiscovered`, `ProvinceOwnershipState.RecentProvinceId`/`RecentOldOwnerId`/`RecentNewOwnerId`, `ProvinceOccupationState.RecentProvinceId`/`RecentOldOccupierId`/`RecentNewOccupierId` **Then** they are excluded from the equality comparison entirely — only the durable dictionary/set content (`OwnerByProvinceId`, `OccupierByProvinceId`, `CountryIds`) determines whether `PropertyChanged` fires. Before implementing, review every call site that sets these transient fields to confirm the durable data they accompany always changes in the same call, so excluding the transient fields from the comparison never silently swallows a distinct event the UI needs to react to (e.g. a toast/flash).
...
- `SaveResultState.Set(...)` — excluded from the equality-check treatment entirely; it continues to fire `PropertyChanged` unconditionally on every call, exactly as today, since each discrete save attempt (even one with an outcome identical to the previous attempt) is a distinct event the UI must react to. Before implementing, review its call sites in `GameLogic.cs` to confirm this stays the only place `Set(...)` is invoked on it.

## Resolved Decisions

Resolved with the user after the initial spec draft (see issue #36 discussion):

- **Transient "recent event" fields** (`DiscoveredCountriesState.RecentlyDiscovered`, `ProvinceOwnershipState.Recent*`, `ProvinceOccupationState.Recent*`): excluded from the equality comparison — only durable dictionary/set content is compared. Implementation must review usages to confirm this is safe (see the Dictionary- and set-holding classes section above).
- **`SaveResultState`**: same answer as the transient fields above — excluded from the equality-check optimization, `Set(...)` keeps firing unconditionally every call (see Out of Scope above).
- **Scope of "each class"**: literally all ~22 `INotifyPropertyChanged` state classes across `VisualState.cs`, `ResourcesState.cs`, and `TimeState.cs`, including `VisualEffectCollection`/`LastFrameEffects` and `GameLogState`.
- **"Minimal overhead" bar**: the qualitative bar is sufficient — the no-op-path benchmarks passing the harness's existing 5%-regression `--compare` gate is the acceptance bar; no bespoke numeric threshold is needed.
