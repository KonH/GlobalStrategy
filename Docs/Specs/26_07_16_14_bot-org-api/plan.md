...
Source: `Docs/Specs/26_07_16_14_bot-org-api/spec.md`.
...
**Behaviour-identity protection:** (a) the moved code is verbatim (no reordering of the three checks, same short-circuits, same first-match resource lookup); (b) new equivalence tests assert `Evaluate`'s verdict matches the pipeline's `ActionValid`/`ActionFailed` outcome across a scenario matrix (see Tests); (c) plan 26_07_16_14_multi-org-headless-simulation's `MultiOrgGameplayTests` (full card pipeline: cost deduction, hand removal, redraw, failure path) run unchanged over the refactored systems.
...
`src/Game.Tests/Game.Tests.csproj` gains `<ProjectReference Include="../Game.Bots/Game.Bots.csproj" />` (the `Game.ConsoleRunner` reference already arrives with plan 26_07_16_14_multi-org-headless-simulation). New bot tests reuse/extend the multi-org `BuildLogic`-style harness plan 26_07_16_14_multi-org-headless-simulation introduces (two orgs, countries, an `ActionConfig` with org + country pools, discover/control effects, monthly income) — extended here with per-card costs/conditions rich enough to drive playability on and off.
...
Test project: `src/Game.Tests/` (xunit, snake_case `[Fact]` names, `StaticConfig<T>`/`BuildLogic` harness style per `GameLogicOrgTests.cs`, extended multi-org builder from plan 26_07_16_14_multi-org-headless-simulation with cost/condition-bearing `ActionConfig`).
...
- **New `src/Game.Tests/ActionPlayabilityTests.cs`** (shared-evaluation behaviour-identity regression):
  ...
  - Plus: plan 26_07_16_14_multi-org-headless-simulation's `MultiOrgGameplayTests` run unchanged over the refactored systems (no edits to those tests — their passing is part of the identity guarantee).
...
- **New `src/Game.Tests/BotProfileTests.cs`** (uses the `Game.ConsoleRunner` reference from plan 26_07_16_14_multi-org-headless-simulation):
...
- *Plan before implement / Spec before plan.* This plan implements the approved `Docs/Specs/26_07_16_14_bot-org-api/spec.md`; no code precedes it, and implementation is additionally gated on plan 26_07_16_14_multi-org-headless-simulation's landed implementation (first agent step).
- *File organisation.* Plan lives at `Docs/Specs/26_07_16_14_bot-org-api/plan.md`, paired with its spec under the same dated identifier. It is ordered by timestamp relative to `Docs/Specs/26_07_16_15_score-component-composition.md`/`Docs/Specs/26_07_16_09_org-scoring/` on `main`.
