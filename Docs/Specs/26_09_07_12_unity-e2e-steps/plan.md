...
**Cursor** — uses `.claude/skills/unity-e2e-run/SKILL.md` directly; no `.cursor/commands/` slash command and no `.cursor/skills/` wrapper. The existing `cursor-meeting-*` wrappers exist solely because meetings need a Cursor-specific session identity; this feature has no Cursor-only override — the file handshake is identical for all three agents.
...
- **Group 13 — three agents:**
  ...
  - `.claude/skills/unity-e2e-run/SKILL.md`, `.agents/skills/unity-e2e-run/SKILL.md` (Cursor uses the Claude skill directly; no slash-command wrapper).
