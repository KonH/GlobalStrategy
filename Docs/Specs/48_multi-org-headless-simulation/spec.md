...
- **Given** an existing save file containing global `IsDiscovered` entries **When** it is loaded **Then** those discoveries are attributed to the save's org (the save header's `OrganizationId`) — old saves must load without error and without losing discovery progress. The save *system* (header, storage, serializer) is otherwise unchanged; only the discovery component's savable shape changes.
