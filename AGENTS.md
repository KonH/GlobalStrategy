...
- **Multi-agent meetings:** `meeting-schedule` / `meeting-join` / `meeting-start` skills — file-based (`Docs/Meetings/`) meetings between Claude, Codex, and Cursor agents on one machine/project; protocol defined in `.claude/skills/meeting-join/SKILL.md`, adapted for Codex under `.agents/skills/meeting-*/`, and mirrored for Cursor as `(CURSOR)`-prefixed `cursor-meeting-*` skills under `.cursor/skills/cursor-meeting-*/` (slash commands `/meeting-*` stay in `.cursor/commands/`)
