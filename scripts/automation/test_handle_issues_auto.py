...
from scripts.automation.handle_issues_auto import (
    MARKER, auto_candidates, park_unroutable, route_candidates, run_provider_handlers,
)
...
class RoutingTests(unittest.TestCase):
    ...
    def test_all_limited_parks_with_need_attention_and_comment(self):
        ...
        with patch("scripts.automation.handle_issues_auto.select_auto_provider",
                   return_value=None), \
             patch("scripts.automation.handle_issues_auto.add_label") as add, \
             patch("scripts.automation.handle_issues_auto.post_comment") as post, \
             patch("scripts.automation.handle_issues_auto.record_auto_selection") as record:
            routed = route_candidates(MagicMock(), Path("state.json"), [candidate])
        self.assertEqual([], routed)
        add.assert_called_once_with(7, "ai-need-attention")
        record.assert_not_called()
        self.assertTrue(post.call_args.args[1].startswith(MARKER))
    ...
    def test_park_unroutable_comment_failure_still_applies_need_attention(self):
        ...
        with patch("scripts.automation.handle_issues_auto.add_label") as add, \
             patch("scripts.automation.handle_issues_auto.post_comment",
                   side_effect=RuntimeError("gh failed")):
            park_unroutable(MagicMock(), candidate)
