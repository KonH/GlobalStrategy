...
## Known pitfall: never background the Ralph loop inside a `claude -p` run

Section 4b of the command drives `scripts/automation/claude/ralph.py` (the implementation loop) from inside a `claude -p` process. That process is one-shot: once it stops making tool calls, it exits, and nothing about it resumes later — there is no notification, no background monitor, no "next turn" the way an interactive session has. A run that launched `ralph.py` via a non-blocking/backgrounded tool call and then said something like "I'll monitor progress and report back once it finishes" was actually done at that point: the process exited seconds later, `ralph.py` never produced a single iteration (empty `.ralph/logs/`, a metrics CSV with only its header row), no commit or PR was created, and the issue was left with `claude-in-progress` applied and an Implementation Proposal comment that looked acted-on but wasn't.

The fix is structural, not a one-off correction: `ralph.py` must be launched as a detached OS process (`nohup ... &`, PID captured to a file) and then polled to completion via repeated short foreground checks, all within the same run — see the command file's section 4b step 3 for the exact pattern. This keeps the `claude -p` process alive (still making tool calls) for as long as `ralph.py` actually takes, instead of exiting the moment the loop is kicked off.

No GitHub-side state needs cleanup after an abandoned run like this: `claude-in-progress` is visibility-only (see above), and section 4b.1 already restarts cleanly from `main` whenever `**Implementation PR:**` isn't set on the checklist — so a stalled attempt with no PR is safe to just re-trigger (a fresh 👍 on the Implementation Proposal comment).
