...
All scripts live in `.claude/` and must be run from the **project root** so relative paths resolve correctly.
...
2. Add an entry to `COUNTRY_FLAGS` in `.claude/download_flags.py`:
   ```python
...
3. If the filename is uncertain, verify it resolves before downloading:
   ```powershell
   .venv\Scripts\python.exe .claude\check_flags.py "File:Flag_of_New_Country_(1850).svg"
   ```
...
```powershell
.venv\Scripts\python.exe .claude\svg_to_png.py path\to\file.svg output.png --width 256
.venv\Scripts\python.exe .claude\svg_to_png.py https://example.com/flag.svg output.png
```
