...
The full-rebuild `Refresh()` above is wrong for a list where entries must independently animate in and out (e.g. a scrolling log). Instead, key each rendered element by a stable identity (e.g. a monotonic `SequenceId`) in a dictionary keyed to a small per-entry record, diff the new state against that dictionary, and only touch what changed: new ids get a fresh element with a short fade-in, ids no longer present get a longer fade-out before removal, everything else is left alone. Fade transitions are driven per-element via `IStyle.transitionDuration` plus `element.style.opacity`.

Do not remove the evicted element with a hardcoded `ExecuteLater(<guessed fade-out duration>)` — a duration guess drifts from the transition's actual effective duration (e.g. the element was already invisible/detached, so no transition runs at all) and either removes the element too early or leaves it lingering. Instead, observe the transition's own lifecycle: register `TransitionRunEvent`, `TransitionEndEvent`, and `TransitionCancelEvent` callbacks on the element *before* writing the opacity that triggers the fade-out, filtered to `evt.target == element` and `evt.stylePropertyNames.Contains("opacity")` (a `StylePropertyNameCollection`, not a plain string). Arm a local `bool` flag only once this fade-out's own `TransitionRunEvent` has been observed, and let the end/cancel handler bail out until armed — this stops a `TransitionCancelEvent` raised by interrupting an in-progress fade-*in* from being mistaken for cancellation of the new fade-*out*. Route end/cancel (and a `DetachFromPanelEvent` safety net) through one idempotent cleanup closure that unregisters all four callbacks and removes the element, tolerating the element or its parent already being detached. Also schedule a next-frame, state-based fallback (`resolvedStyle.opacity` already at the terminal value and no run event observed) — a state check, not a second duration guess — for environments where transition events don't fire reliably. See `ActionLogView` (`Assets/Scripts/Unity/UI/ActionLogView.cs`) for a concrete example, including pausing a still-pending fade-in schedule on eviction before starting fade-out.

### Animation ownership boundary

Three animation mechanisms coexist in this project and must stay separate, never blended:
- **Native USS transitions** (`IStyle.transitionDuration` + a style write, as above) own declarative class/style-driven changes on a `VisualElement`.
- **`UiTween`** (`Assets/Scripts/Unity/UI/UiTween.cs`) is for awaited, imperative `VisualElement` transform interpolation (an explicit coroutine/async animation the caller drives step by step) — not for opacity/style changes a USS transition can express declaratively.
- **`AnimatableInt`/`AnimatableDouble`** and their `AnimationBarrier*` types (see `.claude/rules/animation_barriers.md`) are project-owned display-state infrastructure for gating gameplay/state progression on animation completion — not a general-purpose `VisualElement` animation tool.
