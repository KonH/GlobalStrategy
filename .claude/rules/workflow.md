...
| Unity asset / scene changes | `unity-plugins` skill first (DLL ensure + pin MCP to this checkout), then MCP tools, then `refresh_unity` + `read_console` |
...
Before the first Unity Editor / MCP action in a session, run the `unity-plugins` skill (`python scripts/unity/ensure_plugin_dlls.py`). Pin MCP to this checkout before the first MCP call — another GlobalStrategy Editor is not this project.
