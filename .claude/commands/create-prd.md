...
When `$ARGUMENTS` includes `code-only` or `full-env-headless`, this run has no interactive Unity Editor/MCP connection — it's driven by unattended automation (e.g. `scripts/automation/claude/ralph.py` on a headless host), not a developer session with the Editor open. Adjust task planning accordingly; a run with no marker at all keeps the normal rules above unchanged.

- **`code-only`** — the plan is already confined to `src/` and/or pure C# `Assets/Scripts/` changes with no Unity asset/scene edits. Plan tasks exactly as usual; the existing gates (`dotnet build`/`dotnet test`/python validation) never needed Unity MCP in the first place, so nothing changes.
