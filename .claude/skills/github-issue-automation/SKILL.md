...
- **`claude`** — the owner opted this item in. Its description plus the owner's later comments are the prompt to execute — no required format, no hardcoded spec→plan pipeline; whatever workflow is wanted (e.g. "run /specify for X") goes in the prompt itself.
- **`claude-in-progress`** — a run is actively working the item. Skipped by discovery.
- **`claude-needs-attention`** — the automation stopped and is waiting on the owner (question asked, blocked, partial work, missing environment). Skipped by discovery.
- **`claude-complete`** — the prompt is fully done. Skipped by discovery.
...
1. Adds `claude-in-progress` first.
...
3. Executes it — on the PR's head branch for PR candidates, on `claude/issue-<N>-<short-name>` for issue candidates (reused if it already exists on origin).
...
7. Applies `claude-complete` or `claude-needs-attention`, **then** removes `claude-in-progress` — every item ends the run with exactly one outcome label.

## Crash recovery: stale-label reclaim with a GitHub-side counter

The wrapper holds an exclusive process lock, so at run start no other run can be active — any item still labeled `claude-in-progress` is by definition leftover from a crashed/interrupted run (step 7's ordering guarantees a healthy run never leaves only that label). `reclaim_stale_in_progress` in `issue_handler.py`:
...
- On the crash after that (3rd interrupted attempt), it parks the item with `claude-needs-attention` and an explanatory comment instead — repeated crashes never burn subscription usage forever.
- The reclaim comments *are* the counter — stored on GitHub, no local state. A real owner comment (non-marker, authored by `KonH`) resets it, so a resumed item gets fresh retries.
...
- **Detection (Codex):** unchanged — `usage limit|rate limit|quota exceeded` in error/failed output only (no verified production wall-clock strings).
...
- **Pause orchestration (`handle_limit_pause`):** salvage → always write `Logs/handle_issues_<provider>.limit.json` → on clean/committed: `release_in_progress_silently` (no reclaim comment, no crash-retry) then best-effort automation note on the item (salvage outcome + pause until `retry_at`); on salvage failure: apply `<label>-needs-attention`, remove `<label>-in-progress` **directly** (never via silent release after needs-attention), then best-effort failure comment. Note posting is best-effort *after* save/release so a failed `post_comment` cannot leave `-in-progress` for reclaim. Exits 0.
...
The automation host has no Unity Editor, no Unity MCP, and no image-generation pipeline. Prompts needing those get everything that *is* possible (C# verifiable via `dotnet build`/`dotnet test`, configs, docs, scripts), an explicit list of what was skipped, and a `claude-needs-attention` finish. Implementation tooling like `scripts/automation/claude/ralph.py` remains available as a standalone tool but is no longer wired into this automation.
...
(Same four with the `codex` prefix for the Codex automation.) Only the plain `claude` label is ever applied by the owner when opting an item in — the other three are managed by the automation (removal of `needs-attention`/`complete` to resume being the one owner-side exception).

## Setup checklist

- `gh auth login` on the machine that will run this, authenticated as `KonH`.
- The four `gh label create` commands above (once, on the repo).
