...
Only the plain provider / `auto-ai` labels are ever applied by the owner when opting an item in — the three shared `ai-*` status labels are managed by the automation (removal of `ai-need-attention`/`ai-complete` to resume being the one owner-side exception). When `auto-ai` routing finds every provider usage/session-limited, it parks the item with `ai-need-attention` and an `<!-- auto-ai-automation -->` comment rather than leaving it unlabeled for endless empty polls.
