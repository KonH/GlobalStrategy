...
namespace GS.Game.Tests {
	public class ProvinceOwnershipTests {
		...
		void change_owner_does_not_affect_population() {
			...
			double? PopulationForProvince(string provinceId) {
			...
			double? beforeChange = PopulationForProvince("prov_b");
			Assert.Equal(2000.0, beforeChange);

			ProvinceOwnershipSystem.ChangeOwner(logic.World, "prov_b", "Great_Britain");

			double? afterChange = PopulationForProvince("prov_b");
