...
namespace GS.Game.Tests {
	public class ActionCooldownQueryTests {
		...
		static TestWorld Cooldown(string orgId, string actionId, DateTime endTime) {
			return TestWorld.Create().Entity()
		...
		void is_on_cooldown_true_when_end_time_is_after_current_time() {
			var world = Cooldown("OrgA", "declare_war", Now.AddDays(7));
		...
		void is_on_cooldown_false_when_end_time_equals_current_time() {
			var world = Cooldown("OrgA", "declare_war", Now);
		...
		void is_on_cooldown_false_when_end_time_is_before_current_time() {
			var world = Cooldown("OrgA", "declare_war", Now.AddDays(-1));
		...
		void different_action_ids_for_same_org_do_not_cross_contaminate() {
			var world = Cooldown("OrgA", "declare_war", Now.AddDays(7))
				.Entity().With(new ActionCooldownState {
					OrgId = "OrgA", ActionId = "make_friend", EndTime = Now.AddDays(-1)
				});
		...
		void different_orgs_for_same_action_id_do_not_cross_contaminate() {
			var world = Cooldown("OrgA", "declare_war", Now.AddDays(7))
				.Entity().With(new ActionCooldownState {
					OrgId = "OrgB", ActionId = "declare_war", EndTime = Now.AddDays(-1)
				});
