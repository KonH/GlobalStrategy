...
namespace GS.Unity.UI {
	class WarProgressLayoutBinder {
		...
		public WarProgressLayoutBinder(
		...
		public void Refresh(
			...
			if (_title != null) {
				_title.text = string.Format(
					...
					GetCountryName(attackerCountryId),
					GetCountryName(defenderCountryId));
			}

			UpdateProgressBar(progress);
			UpdateFlag(_attackerFlag, attackerCountryId);
			UpdateFlag(_defenderFlag, defenderCountryId);
			RebuildEffectsList(history, battles);
			UpdateSideStats(attacker, _attackerRecruits, _attackerTroopsInBattles, _attackerCasualties, _attackerDamage, _attackerDurability, "attacker");
			UpdateSideStats(defender, _defenderRecruits, _defenderTroopsInBattles, _defenderCasualties, _defenderDamage, _defenderDurability, "defender");
			RebuildBattlesList(battles);
		}
		...
		void RebuildEffectsList(
			...
			foreach (WarBattleRowState battle in battles) {
			...
			foreach (WarProgressHistoryEntryState entry in history) {
		...
		void RebuildBattlesList(IReadOnlyList<WarBattleRowState> battles) {
			if (_battlesList == null) {
				return;
			}
