...
- **Web client terminal commands:** `add-terminal-command` skill — checklist for keeping the debug terminal Tab completion and ECS inspector domain-id dropdowns working when adding or changing an `ICommand` type in `src/Game.Commands` **or** a domain-id field on `src/Game.Components` (attributes live in `src/Game.Common/ParamSuggestion.cs`; providers live in `src/Game.Commands.Text`)
