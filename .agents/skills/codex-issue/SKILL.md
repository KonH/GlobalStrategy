...
- `ai-in-progress` — a run is actively working it (discovery skips it). Shared across providers. **Owned exclusively by the Python wrapper** — never add or remove it from the agent, including manual invocations (the operator/wrapper manages the label).
...
The wrapper already claimed the item (`ai-in-progress` via `claim_candidate`) before invoking you, and will clear that label after the CLI returns. Take the candidate through all steps, in order:
...
3. **Always commit and push** whatever artifacts exist — even partial or incomplete work — using the local `commit` skill (which applies `.claude/commands/commit.md` and delegates generic mechanics to `cd:commit`), then `git push -u origin <branch>`. Never leave work unpushed and never discard partial work: the pushed branch is the next run's starting point.
