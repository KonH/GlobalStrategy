...
When the plan will touch Unity, run the `unity-plugins` skill first. When changes touch `src/`, finish with the project `dotnet-build` skill in Release mode as required by `.claude/rules/workflow.md` (do not commit plugin DLLs).
