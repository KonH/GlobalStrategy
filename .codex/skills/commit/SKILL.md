...
4. Bump `Assets/Settings/Build Profiles/Web - Desktop - Release.asset` to the same value and stage it. That profile embeds a full `m_PlayerSettingsYaml` snapshot of PlayerSettings whose `bundleVersion` wins for every profile-driven Web build; if the override is still present, edit its `- line: '|   bundleVersion: X.YYY'` entry, or published WebGL builds keep shipping the stale version.
