...
4. Check `git diff --cached --name-only` for any staged path under `src/`. If there is one, the `Assets/Plugins/Core/*.dll` files are stale: run `dotnet build src/GlobalStrategy.Core.sln -c Release > .tmp/dotnet-build.log 2>&1` (no `cd`), read the log, stop and report on failure, otherwise `git add Assets/Plugins/Core/*.dll` and delete the log. Skip this step if no staged path is under `src/`.
5. Run `python scripts/stats/collect_usage.py --scan` once. Treat failures as non-blocking: report them, then continue.
6. Follow the available `k:commit` skill to select a branch if necessary, review the staged scope, choose an intentional commit message, and create the commit.
